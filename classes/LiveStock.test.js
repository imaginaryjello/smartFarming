...
const LiveStock_1 = require("./LiveStock");
describe("Livestock Class", () => {
    let livestock;
    beforeEach(() => {
        livestock = new LiveStock_1.Livestock("L001", "Healthy", "Barn A", "Standard Diet");
    });
    ...
    it("should monitor health status", () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith("Monitoring health status of livestock L001");
    });
    it("should feed according to the diet plan", () => {
        const consoleSpy = jest.spyOn(console, "log"); // Spy on console.log
        livestock.feed();
        expect(consoleSpy).toHaveBeenCalledWith("Feeding livestock L001 with diet plan: Standard Diet");
    });
});
